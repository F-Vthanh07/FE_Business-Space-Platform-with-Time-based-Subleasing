...
export const RenterOverview: React.FC = () => {
  const [activeMarketTab, setActiveMarketTab] = useState<'24h' | '7d'>('24h');
  const { t, language } = useThemeLanguage();

  // Dữ liệu biểu đồ Live Flow của Renter
  const liveFlowBars = [
    { height: '25%' },
    { height: '40%' },
    { height: '65%' },
  ...
  return (
    <div className="renter-overview animate-in">
      <div className="dashboard-grid">
        
        {/* CỘT TRÁI - GỒM THẺ HERO VÀ THẺ MARKET SENTIMENT */}
        <div className="grid-left-col">
          
          {/* THẺ HERO: Sublease Architecture */}
          <div className="glass-card hero-card">
            <span className="label-caps label-hero">{t('overview.subleaseArchitecture')}</span>
            <h1 className="hero-title">{t('overview.tactileAtmosphere')}</h1>
            <p className="hero-desc text-secondary">
              {t('overview.subleaseDesc')}
            </p>
            ...
            <div className="hero-inset-cards">
              ...
              <div className="glass-card--inset inset-stat-card">
                <span className="label-caps inset-card-label">{t('overview.subleaseRevenue')}</span>
                <h2 className="inset-card-value">
                  {language === 'en' ? '8.65M VND' : '8,65tr ₫'}
                </h2>
              ...
              {/* Card 2: ACTIVE SLOTS */}
              <div className="glass-card--inset inset-stat-card">
                <span className="label-caps inset-card-label">{t('overview.activeSlots')}</span>
                <h2 className="inset-card-value">14 <span className="value-unit">Slots</span></h2>
          ...
          <div className="glass-card market-card">
            <div className="market-card-header">
              <h2 className="market-title">{t('overview.subleaseSentiment')}</h2>
            ...
            <div className="market-list">
              ...
              <div className="glass-card--inset market-row">
                ...
                <div className="market-row-right">
                  <span className="coin-price">{language === 'en' ? '12.5M VND' : '12.5tr ₫'}</span>
              ...
              <div className="glass-card--inset market-row">
                <div className="market-row-left">
                  ...
                  <div className="coin-meta">
                    <span className="coin-name">{language === 'en' ? 'Phan Dinh Phung' : 'Phan Đình Phùng'}</span>
                    <span className="coin-symbol">{language === 'en' ? '30 m² / Cafe' : '30 m² / Cà phê'}</span>
                  </div>
                </div>
                <div className="market-row-right">
                  <span className="coin-price">{language === 'en' ? '8.2M VND' : '8.2tr ₫'}</span>
                  <span className="coin-change text-positive">{language === 'en' ? '78% occupancy' : '78% lấp đầy'}</span>
                </div>
              </div>
              ...
              <div className="glass-card--inset market-row">
                ...
                <div className="market-row-right">
                  <span className="coin-price">{language === 'en' ? '3.8M VND' : '3.8tr ₫'}</span>
                  <span className="coin-change text-negative">{language === 'en' ? '45% occupancy' : '45% lấp đầy'}</span>
                </div>
              </div>
            ...
            <div className="glass-card square-card">
              ...
              <div className="square-card-content">
                ...
                <span className="square-card-value">
                  {language === 'en' ? '1 Active' : '1 Hoạt động'}
                </span>
              </div>
            </div>
